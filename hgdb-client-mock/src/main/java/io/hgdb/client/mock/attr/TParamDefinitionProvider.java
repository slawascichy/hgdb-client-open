 ...
 * 
 */
public class TParamDefinitionProvider extends
		TAbstractProvider<ParamDefinition> {
	...
	protected ParamDefinition rowMapper(String[] row) {
		String[] header = new String[] { "definitionName", "version",
				"description", "isLatestVersion", "htmlControlName",
				"paramType", "timeToLiveSeconds", "valueDefinition",
				"sourceType", "source", "sourceJndiName", "sourceJ2CName",
				"presentationUrlPrefix", "defaultValue", "fromDate", "toDate",
				"complexClass", "isIndexable", "alternateName",
				"recomendedLabel", "subType" };
	...
	protected ParamDefinition rowMapper(String[] row, String[] header) {
		...
		if (row != null && header != null && row.length == header.length) {
			...
			this.logger.error(msg, new Object[] { ArrayUtils.toString(row),
					ArrayUtils.toString(header) });
			throw new NotImplementedException(
					"Nie wczytano danych dla wiersza row="
							+ ArrayUtils.toString(row)
							+ " oraz naglowka header="
							+ ArrayUtils.toString(header));
		}
	...
	private void setPropertyValue(String property, String newValue,
		...
		if (property.equalsIgnoreCase("definitionName")) {
			...
			paramDefinition.getId().setVersion(
					MockDataUtils.convertToLong(newValue));
			...
			paramDefinition.setIsLatestVersion(MockDataUtils
			...
			paramDefinition.setTimeToLiveSeconds(MockDataUtils
			...
			paramDefinition.setFromDate(MockDataUtils
					.convertToCalendar(newValue));
		} else if (property.equalsIgnoreCase("toDate")) {
			if (StringUtils.isNotBlank(newValue)) {
				paramDefinition.setToDate(MockDataUtils
						.convertToCalendar(newValue));
			}
			...
			paramDefinition.setIsIndexable(MockDataUtils
					.convertToBoolean(newValue));
			...
			String msg = "Obiekt ParamDefinition nie posiada właściwości property="
					+ property;
